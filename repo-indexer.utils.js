export function chunkJavaByMethods_OLD(source) {
  ...
  function cleanLineForDetection(line) {
    let result = '';
    let i = 0;
  ...
  function isConstructor(line) {
    ...
    const pattern = new RegExp(`\\b${className}\\s*\\(`);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const cleaned = cleanLineForDetection(rawLine);
    isClassDeclaration(cleaned);

    const { open, close } = countBraces(rawLine);

    if (!inMethod) {

      const isStaticBlock = isStaticInitializer(cleaned);
      const isCtor = isConstructor(cleaned);
      const isMethod = isMethodSignature(cleaned);

      if (isStaticBlock || isCtor || isMethod) {

        let start = i;

        // include annotations above
        while (start > 0) {
          const prevClean = cleanLineForDetection(lines[start - 1]);
          if (isAnnotation(prevClean)) {
            start--;
          } else {
            break;
          }
        ...
        if (isStaticBlock) {
          methodName = "static_initializer";
        } else if (isCtor) {
          methodName = className;
        } else {
          methodName = extractMethodName(cleaned);
        }
      }
      ...
      if (braceDepth === 0) {
        let end = trimTrailingEmpty(i);

        chunks.push({
          chunk_id: chunkId++,
          function_name: methodName,
          start_line: methodStart + 1,
          end_line: end + 1,
          text: lines.slice(methodStart, end + 1).join('\n')
        });
